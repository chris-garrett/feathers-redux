...
const { default: reduxifyServices, getServicesStatus, fromJS } = require('../../src/immutablejs');

const initServiceState = fromJS({
  isError: null,
  isLoading: false,
  isSaving: false,
  isFinished: false,
...
const savingStatus = fromJS({
  message: 'messages is saving',
  className: 'isSaving',
  serviceName: 'messages'
});
const errorStatus = id => (fromJS({
...
describe('integration test (immutable.js)', () => {
  describe('feathers-redux', () => {
    ...
    it('initial state', () => {
      ...
      assert.deepEqual(state.get('users'), initServiceState);
      assert.deepEqual(state.get('messages'), initServiceState);
    ...
    it('failed service call', () => {
      ...
      return promise
        ...
        .catch(() => {
          state = store.getState();
          assert.equal(state.getIn(['messages', 'isError', 'className']), 'not-found');
          assert.deepEqual(
            state.get('messages').set('isError', null),
            initServiceState.set('isError', null).set('isFinished', true).set('data', null)
          );
